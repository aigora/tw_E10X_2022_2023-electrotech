...
/* DESDE AQUI 04/05/2023

void Fichero1(int a)  //ESTAN SEPARADOS POR ,
{

    char aux0, aux1;
    int contador=0, i, j=0; //para saber cuantas
    int numerodefilas=0;
    int numerodeenergias; //SERÁ (NUMERO DE FILAS DE FICHERO - 5 PRIMERAS LINEAS)


    //LO ABRIMOS POR PRIMERA VEZ PARA DETERMINAR LA DIMENSIÓN DE NUESTRO FICHERO
    ...
    else
    {
     ...
     numerodefilas++; //LE SUMAMOS LA ULTIMA FILA QUE NO TIENE \n, YA QUE TIENE EL EOF

     fclose(pf); //CERRAMOS FICHERO DENTRO DEL ELSE
    }
    ...
    FILE *pf; //esto no se si se puede hacer, o hay que poner que el puntero apunte otra vez al principio y ya en vez de abrir y cerrar el fichero

    pf = fopen("generacion_por_tecnologias_21_22.csv", "r"); //abrimos un fichero que se llama pf para almacenar los datos
    ...
    else //aqui vamos a almacenar los datos
    {

    //primero voy a recorrer las 5 primeras lineas guardandolas en una variable auxiliar que me la pela

    for(i=0; i<=5; i++)
    {
        while(fscanf(pf, "%c", aux1) != '\n')
        {
            if(aux1=='\n')
                contador++;
        }
    ...
    //Aqui ya he recorrido las líneas que no me interesan y la barra esta justo al inicio de la línea 6, hay tipo de energia y 24 datos


    //AHORA PARA TOMAR LOS DATOS Y EL TIPO DE ENERGIA A LA VEZ, VOY A EMPLEAR UN BUCLE FOR.
    //PARA ELLO tengo que determinar la dimensión del fichero --> con un while


    //¿LO TENGO QUE HACER EL PRINCIPIO CUANDO YA HAYA RECORRIDO EL FICHERO UNA VEZ, ¿Y LUEGO LO TENGO QUE VOLVER A ABRIR?


    numerodeenergias=numerodefilas-5; //le restamos las 5 primeras filas

    Energia Energias[numerodeenergias]; //la extensión de los datos es la calculada en la línea anterior.

    j=0; //ESTO ES PARA EL BUCLE WHILE DE LOS DATOS


    for(i=0; i<= numerodeenergias; i++) //NOSE COMO PONER EL NUMERO DE FILAS
    {

        fscanf(pf, "[^,]", Energias[i].tipo); //guardará la cadena de caracteres del fichero hasta la ',' que es como se encuentra separado el fichero

        //para leer el resto de datos, simplemente recorro los datos del fichero hasta el \n

        while(fscanf(pf, "%f", Energias[i].datos[j]) != '\n')
        {
            j++;
        }
...
//Faltan por asignar las fechas.


}

HASTA AQUI LA NUEVA MODIFICACIÓN
*/
